...
display_post_installation() {
    ...
    print_header "🔧 Optional Tools (run these if needed):"
    echo ""
    echo "• Python environment (pyenv + Python 3.12/3.13):"
    echo "  cd $SCRIPT_DIR && ./setup-helpers/05-install-python.sh"
    echo ""
    echo "• Node.js environment (Volta):"
    echo "  cd $SCRIPT_DIR && ./setup-helpers/06-install-nodejs.sh"
    echo ""
    echo "• Database containers (PostgreSQL, MySQL, Redis):"
    echo "  cd $SCRIPT_DIR && ./setup-helpers/07-setup-databases.sh"
    echo ""
    echo "• AI/ML tools (Ollama, etc.):"
    echo "  cd $SCRIPT_DIR && ./setup-helpers/09-install-ai-ml-tools.sh"
    echo ""
